...
# Check to see how many unique valies there are for "patient_category"
...
ncol(GSE212041_data) - 1 # 781, first colunmn has "Symbol", not sample data 

# check that sample ids are the same in data and metadata
length( intersect(rownames(GSE212041_metadata), colnames(GSE212041_data)) ) #781
identical(rownames(GSE212041_metadata), colnames(GSE212041_data)) # not ordered the same
dim(GSE212041_metadata) # 781 15
dim(GSE212041_data) # 60640 782 -- metadata missing from a single column in the data
setdiff(colnames(GSE212041_data), rownames(GSE212041_metadata)) # "Symbol" is the column that does not exist in the metadata
colnames(GSE212041_data)
# See what "Symbol" is --  
GSE212041_data[1:3,1:3] # looks like an additional ID for the ENSG number? Not entirely sure 
# But need to remove this column to proceed with analysis.
...
length( intersect(rownames(GSE212041_metadata), colnames(GSE212041_data)) )
identical(rownames(GSE212041_metadata), colnames(GSE212041_data)) # order must be different
...
identical(rownames(GSE212041_metadata), colnames(GSE212041_data)) # now it is TRUE, save both 
...
hist(log10(all_GSE212041_data), breaks =100) # teh data appear to be roughly log normal
# with ggplot
...
plot(GSE212041_raw_median, pch=20) # a few outliers, roughly those above 1; data are a little noisy
...
plot(GSE212041_raw_sd, pch=20) # values above 1500 are somewhat anamalous 
...
hist(log10(all_GSE212041_data_n), breaks =100) # no longer look log normal
...
plot(GSE212041_norm_max) # given treatment, should all have max of 1? There are some below 1
...
# Have selected to use "patient_category" as the group designator for the statistical test below.
...
# does not help with the horseshoe
...
# produce a heatmap dendrogram of the statistically subselected data 
