...
# differences between/amoung groups that were unambiguously labeled as testing positive for covid or
...
dir_path <- "~/Downloads/combine_covid/"
...
# GSE198449
GSE198449_stat <- import_data("~/Downloads/GSE198449/GSE198449_stat_results_subselected_and_cleaned.txt")
...
# GSE215865
GSE215865_stat <- import_data("~/Downloads/GSE215865/GSE215865_stat_results_subselected_and_cleaned.txt")
# The rownames of this dataset contain the ENSG stable gene IDs along with a version tag. The tag is removed
# to allow for comparison of just the stable gene ID
...
# GSE212041
GSE212041_stat <- import_data("~/Downloads/GSE212041/GSE212041_stat_results_subselected_and_cleaned.txt")
# The ronames of this dataset contain the ENSG stable gene ID WITHOUT a version tag. No modification
# necessary.
...
length(first_pair) # 180
# now among all three analyses
all_three <- intersect( first_pair,rownames(GSE212041_stat) ) # 71
length(all_three) # 74
...
annotations <- read.table(file="~/Downloads/GSE198449/mart_export.txt",row.names=NULL,header=TRUE,sep="\t", #changed row.names from 1 to NULL
...
# Check to make sure that we still have 74 genes
dim(my_results) # 73 -- one wasn't annotated. Figure out which one it was.
...
GSE198449_metadata <- import_metadata("~/Downloads/GSE198449/GSE198449.metadata.txt")
...
GSE215865_metadata <- import_metadata("~/Downloads/GSE215865/GSE215865.metadata.txt")
...
GSE212041_metadata <- import_metadata("~/Downloads/GSE212041/GSE212041.metadata.txt")
...
my_stable_gene_ids <- all_three

# Use the gprofiler2 pacakge to perform preliminary pathway analysis
gostres <- gost(query = my_stable_gene_ids, organism = 'hsapiens', significant = TRUE)
